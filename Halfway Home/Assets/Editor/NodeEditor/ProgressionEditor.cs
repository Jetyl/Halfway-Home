...
public class ProgressionEditor : BaseNodeEditor
{
    [SerializeField]
    public JsonData ProgressionData;
    ...
    private static void OpenWindow()
    {
        ...
        window.titleContent = new GUIContent("Multi-Progression Check");
    }
    ...
    protected void OnClickAddEndNode(Vector2 mousePosition)
    {
        if (nodes == null)
        {
            nodes = new List<BaseNode>();
    ...
    protected void OnClickAddChangeNode(Vector2 mousePosition)
    {
        if (nodes == null)
        {
            nodes = new List<BaseNode>();
    ...
    public void LoadInfo(JsonData pro)
    {
        ...
        for (int i = 0; i < ProgressionData.Count; ++i)
        {
            ...
            double x = (double)ProgressionData[i]["NodePositionX"];
            double y = (double)ProgressionData[i]["NodePositionY"];
            ...
            if (NewID < (int)ProgressionData[i]["ID"])
                NewID = (int)ProgressionData[i]["ID"];
        ...
        for (int i = 0; i < nodes.Count; ++i)
        {
            ...
            switch (nodes[i].TypeID)
            {
                case NodeTypes.StartNode:
                    ...
                    {
                        if ((nodes[k]).ID == ((StartNode)nodes[i]).NextID)
                        {
                            connections.Add(new Connection(nodes[k].inPoint, ((StartNode)nodes[i]).outPoint, OnClickRemoveConnection));
                        }
                    }
                    break;
                case NodeTypes.ProgressNode:
...
#if UNITY_EDITOR
        path = "Assets/Resources/Progress/" + ChainName + ".json";
#endif
